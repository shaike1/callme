...
const defaultSettings = {
  ...
  aiEngine: process.env.AI_ENGINE || 'ocplatform',    // 'gemini-live' | 'groq-pipeline' | 'openai-realtime' | 'ocplatform'
  ...
  deepgramApiKey: process.env.DEEPGRAM_API_KEY || '',  // Deepgram API key for OCPlatform/Groq STT
...
app.post('/api/chat', async (req, res) => {
  ...
  let systemPrompt = botSettings.persona || 'You are a helpful voice assistant. Always respond in Hebrew.';
  if (integrations.ha?.enabled && integrations.ha?.url) {
    systemPrompt += '\n\nYou can control smart home devices via HA actions.';
  }

  // Try OCPlatform brain first, fallback to Gemini
  const ocpUrl = (integrations.ocplatform?.enabled && integrations.ocplatform?.url)
    ? integrations.ocplatform.url
    : process.env.OCPLATFORM_GATEWAY_URL;

  if (ocpUrl) {
    logger.info("[DEBUG] Starting OCPlatform request", { ocpUrl });
    try {
      ...
      if (rawBody.includes('data: ')) {
        // SSE format: parse each data: line
      ...
      if (text) {
        ...
        return res.json({ success: true, response: text, sessionId, engine: 'ocplatform' });
      }
    } catch (err) {
      logger.warn('OCPlatform chat failed, trying Gemini', { error: err.message });
    }
  }

  // Fallback: Gemini
  const apiKey = botSettings.geminiApiKey;
  if (!apiKey) return res.status(500).json({ error: 'No AI engine configured (OCPlatform unavailable, GEMINI_API_KEY not set)' });
  ...
  try {
    ...
    logger.info('API chat via Gemini', { sessionId, responseLength: text.length });
    res.json({ success: true, response: text, sessionId, engine: 'gemini' });
  } catch (err) {
...
// Connect to Drachtio (optional — skip if DRACHTIO_DISABLED=true or no SIP config)
const drachtioDisabled = process.env.DRACHTIO_DISABLED === 'true' || !process.env.SIP_EXTENSION;
if (drachtioDisabled) {
  logger.info('Drachtio disabled — running in API-only mode (no SIP)');
} else {
srf.connect({
...
} // end if (!drachtioDisabled)

logger.info('Voice worker v2 started', {
  version: '2.0.0',
  mode: drachtioDisabled ? 'api-only' : 'full-sip',
  drachtioHost: drachtioDisabled ? 'disabled' : config.drachtio.host,
